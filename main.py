...
def create_cartesia_tts(language: str, speed: float = 1.0):
    ...
    Использует модель sonic-3.5 для всех языков
    """
...
class Assistant(Agent):
    ...
    async def update_llm_instructions(self):
        """Пересобирает системные инструкции (язык, recap, player_state) и применяет их к LLM."""
        try:
    ...
    async def _publish_tool_event(self, tool: str, payload: dict):
        """Публикует событие вызова tool'а в data channel под топиком 'agent_event'.
        Фронтенд просто логирует это в console.log для дебага."""
        try:
            msg = json.dumps({
                "type": "tool_event",
                "tool": tool,
    ...
    async def update_player_state(
        ...
        # Перегенерируем системные инструкции, чтобы следующий ход видел свежее состояние
        await self.update_llm_instructions()
